...
Nexus Agent Model Hub 是一套面向多租户登录、Agent 工具调用、大模型百科、排行榜与对话演示场景的源码开放参考仓库，提供双语界面、双语文档、Docker 部署资产与可扩展插件能力。
...
![登录、控制台、模型库与对话演示预览](../assets/quick-preview.svg)
...
- 数据库：生产环境使用 PostgreSQL，本地开发才允许 SQLite 作为兜底选项
...
1. 注册一个租户管理员，或使用预置演示账号登录。
2. 进入控制台，查看当前租户数据。
3. 浏览 `/models`、`/leaderboard`、`/compare`、`/chat` 与 `/settings` 页面。
4. 在控制台 API 演示中调用内置 `echo` 或 `calculator` Agent。
