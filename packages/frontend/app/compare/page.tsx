...
function ComparePageClient() {
  ...
  const [selected, setSelected] = useState<string[]>(['gpt-4o', 'claude-sonnet-4-20250514']);
  const didApplyQueryRef = useRef(false);
  const picks = useMemo(() => modelCatalog.filter((model) => selected.includes(model.id)), [selected]);
  const cheapestInput = picks.length > 0 ? [...picks].sort((left, right) => left.pricing.inputPer1M - right.pricing.inputPer1M)[0] : null;
  const maxContext = picks.length > 0 ? [...picks].sort((left, right) => right.contextWindow - left.contextWindow)[0] : null;
  ...
  useEffect(() => {
    ...
    const next = requested
      .map((pick) => modelCatalog.find((model) => model.id === pick || model.slug === pick)?.id)
    ...
    <main className="shell stack">
      <section className="panel stack page-hero">
        ...
        <div className="stat-grid">
          ...
          <div className="stat-card">
            <span>最低输入价格</span>
            <strong>{cheapestInput ? formatPrice(cheapestInput.pricing.inputPer1M) : '-'}</strong>
          </div>
          <div className="stat-card">
            <span>最大上下文</span>
            <strong>{maxContext ? formatNumber(maxContext.contextWindow) : '-'}</strong>
          </div>
          <div className="stat-card">
            <span>推荐数量</span>
            <strong>2 - 4</strong>
        ...
        <div className="toolbar">
          <button className="ghost" type="button" onClick={() => setSelected(['gpt-4o', 'claude-sonnet-4-20250514'])}>
            重置默认
          </button>
      ...
      {picks.length < 2 ? (
        <section className="panel empty-state">
          ...
          <p className="fine">详情页里的“加入对比”链接现在已经可生效，也可以在这里继续手动追加模型。</p>
        </section>
      ) : (
